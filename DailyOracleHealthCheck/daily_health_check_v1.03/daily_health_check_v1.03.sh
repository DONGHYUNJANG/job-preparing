...
PROMPT
PROMPT [9. Blocking Session and Kill Command List]
...
# 10. 사용자 세션 및 백그라운드 작업 점검 (Parallel, Job 포함)
sqlplus -s / as sysdba << EOF >> $REPORT_FILE
SET FEEDBACK OFF
SET PAGESIZE 100
SET LINESIZE 250
...
PROMPT
PROMPT [10. User Sessions and User-Invoked Background Jobs]
COLUMN TYPE        FORMAT a10 HEADING 'TYPE'
COLUMN SID_SERIAL  FORMAT a12 HEADING 'SID,SER#'
COLUMN STATUS      FORMAT a10 HEADING 'STATUS'
COLUMN USERNAME    FORMAT a12 HEADING 'USER'
COLUMN PROGRAM     FORMAT a25 HEADING 'PROGRAM'
COLUMN TIME_MIN    FORMAT 999,990 HEADING 'MINS'

SELECT CASE 
         WHEN s.program LIKE '%(P%)' THEN 'PARALLEL'  -- 사용자의 병렬 프로세스
         WHEN s.program LIKE '%(J%)' THEN 'USER_JOB'  -- 사용자의 스케줄러 작업
         ELSE 'USER_SESS'                             -- 일반 접속 세션
       END AS TYPE,
       s.sid || ',' || s.serial# AS SID_SERIAL,
       s.status,
       s.username,
       SUBSTR(s.program, 1, 25) AS PROGRAM,
       ROUND(s.last_call_et / 60, 1) AS TIME_MIN,
       SUBSTR(q.sql_text, 1, 60) AS SQL_TEXT
FROM v\$session s
LEFT JOIN v\$sql q ON s.sql_id = q.sql_id
WHERE s.username IS NOT NULL                 -- 오라클 순수 BG 제외
  AND s.username NOT IN ('SYS', 'SYSTEM')    -- 관리자 세션 제외
  AND (
       (s.status = 'ACTIVE' AND s.last_call_et > 1800)      -- 30분 이상 실행 중
    OR (s.status = 'INACTIVE' AND s.last_call_et > 10800)   -- 3시간 이상 유휴
  )
ORDER BY TYPE, s.last_call_et DESC;

EXIT;
EOF
# 11. Redo Log Switch Frequency (Last 1 Day)
sqlplus -s / as sysdba << EOF >> $REPORT_FILE
SET FEEDBACK OFF
SET PAGESIZE 100
SET LINESIZE 150
...
PROMPT
PROMPT [11. Redo Log Switch Frequency (Last 24 Hours)]
COLUMN SWITCH_HOUR FORMAT a20 HEADING 'TIME(HOUR)'
COLUMN SWITCH_COUNT FORMAT 999,999 HEADING 'SWITCH_COUNT'
COLUMN STATUS FORMAT a10 HEADING 'STATUS'

SELECT TO_CHAR(first_time, 'YYYY-MM-DD HH24') AS SWITCH_HOUR,
       COUNT(*) AS SWITCH_COUNT,
       CASE WHEN COUNT(*) > 4 THEN 'WARNING(>4)' ELSE 'OK' END AS STATUS
FROM v\$log_history
...
PROMPT
PROMPT [12. ASM Diskgroup Usage]
COLUMN NAME FORMAT a20 HEADING 'DISKGROUP_NAME'
COLUMN TOTAL_GB FORMAT 999,999.99 HEADING 'TOTAL_GB'
COLUMN FREE_GB FORMAT 999,999.99 HEADING 'FREE_GB'
COLUMN USED_PCT FORMAT 990.99 HEADING 'USED_PCT'
