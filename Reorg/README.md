...
> **‼️ 매우 중요: 경고 ‼️**
>
> *   **데이터가드(Data Guard) 환경 주의**: 이 스크립트는 `ALTER TABLE ... MOVE` 작업을 수행합니다. Data Guard 환경에서는 데이터베이스가 `FORCE LOGGING` 모드로 운영되는지 반드시 확인해야 합니다. `NOLOGGING` 또는 `UNLOGGED` 작업은 Standby 데이터베이스에 반영되지 않아 데이터 불일치 및 손상을 유발할 수 있습니다.
...
*   `reorg_pre_check.sql`
    *   Reorg 작업을 시작하기 전 시스템 자원(CPU, Undo, Archive, Temp, PGA)이 충분한지 확인하는 진단용 SQL 스크립트입니다.
*   `reorg_execute.sh`
    *   테이블 재구성(`MOVE`)과 인덱스 재빌드(`REBUILD`)를 병렬로 실행하고, 작업 후 인덱스/객체 유효성을 검증하며, 마지막으로 스키마 통계를 재수집하는 메인 셸 스크립트입니다.
...
@reorg_pre_check.sql
```
*   **확인 사항**: Undo, Temp, Archive 공간이 충분한지, CPU 사용률이 너무 높지 않은지 등을 반드시 확인합니다. 자원이 부족하면 작업을 진행해서는 안 됩니다.
...
터미널(셸)에서 `reorg_execute.sh` 스크립트를 실행합니다. 이 스크립트는 테이블/인덱스 재구성, 유효성 검증, 통계 수집을 모두 수행합니다. 작업 내용은 `reorg_execute_*.log` 파일에 기록됩니다.
